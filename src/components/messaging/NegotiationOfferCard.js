...
export function renderNegotiationOfferCard(offer = {}, { isCurrentUserSeller = false } = {}) {
  const currentUser = store.getCurrentUser() || {};
  const isMine = offer.proposedById === currentUser.id;
  const proposer = store.getUserById(offer.proposedById) || { fullName: 'Student' };
  const listing = store.getListingById(offer.listingId) || {};
  const currency = offer.currency || listing.currency || 'INR';
    ...
    >
      ...
      <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
        <div style="display: flex; align-items: center; gap: 6px;">
          ...
          <span class="utility-label" style="font-size: 11px; color: var(--on-surface-variant);">
            ${isMine ? 'Your Offer' : `Offer from ${escapeHtml(proposer.fullName)}`}
          </span>
        </div>
      ...
      <div style="text-align: center; margin: 16px 0;">
        ...
        <div style="font-size: 12px; color: var(--on-surface-variant); margin-top: 4px;">
          Original Price: ${formatPrice(listing.price || 0, currency)}
        </div>
      </div>
