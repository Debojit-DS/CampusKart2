...
export function renderConversationListItem(conversation = {}, { isActive = false } = {}) {
  ...
  // Backend returns buyer/seller embedded
  const otherUser = conversation.buyerId === currentUserId
    ? conversation.seller
    : conversation.buyer;
  const otherName = otherUser?.fullName || 'Student';

  // Backend returns listing embedded
  const listing = conversation.listing || { title: 'Listing', images: [] };
  const mainImage = listing.images?.[0]?.url || 'https://images.unsplash.com/photo-1581291518857-4e27b48ff24e?w=200';
  const hasUnread = (conversation.unreadCountForCurrentUser || 0) > 0;
