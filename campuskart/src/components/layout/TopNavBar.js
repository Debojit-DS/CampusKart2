...
export function renderTopNavBar({ activeCategory = '', activeRoute = '' } = {}) {
  ...
  // Trigger async unread count refresh (will update DOM when data arrives)
  if (isAuth) {
    setTimeout(() => {
      store.refreshUnreadCounts().then(() => {
        // Update notification badge
        const notifBadge = document.querySelector('[data-unread="notifications"]');
        if (notifBadge) {
          const count = store.getUnreadNotificationCount();
          notifBadge.innerHTML = count > 0 ? `<span style="width: 8px; height: 8px; border-radius: 50%; background: var(--primary); display: block;"></span>` : '';
        }
        // Update message badge
        const msgBadge = document.querySelector('[data-unread="messages"]');
        if (msgBadge) {
          const count = store.getUnreadMessageCount();
          msgBadge.innerHTML = count > 0 ? `<span style="width: 8px; height: 8px; border-radius: 50%; background: var(--primary); display: block;"></span>` : '';
        }
      }).catch(() => {
    ...
    <header class="top-nav-bar" style="position: sticky; top: 0; z-index: 50; height: 64px; background-color: var(--surface); border-bottom: 1px solid var(--surface-container); display: flex; align-items: center; justify-content: space-between; padding: 0 var(--margin-mobile); box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
      ...
      <div style="display: flex; align-items: center; gap: 10px;">
        ...
        ${isAuth ? `
          ...
          <!-- Wishlist -->
          <a href="#/wishlist" class="btn-icon btn-ghost" title="Wishlist" aria-label="Wishlist" style="position: relative;">
            <span class="material-symbols-outlined" style="${activeRoute === '/wishlist' ? 'color: var(--primary);' : ''}">search</span>
          </a>

          <!-- Leaderboard -->
          <a href="#/leaderboard" class="btn-icon btn-ghost" title="Leaderboard" aria-label="Leaderboard" style="position: relative;">
            <span class="material-symbols-outlined" style="${activeRoute === '/leaderboard' ? 'color: var(--primary);' : ''}">leaderboard</span>
          </a>
          ...
          <a href="#/messages" class="btn-icon btn-ghost" title="Messages" aria-label="Messages" style="position: relative;">
            ...
            <span data-unread="messages" style="position: absolute; top: 6px; right: 6px;">
              ${unreadMessages > 0 ? `<span style="width: 8px; height: 8px; border-radius: 50%; background: var(--primary); display: block;"></span>` : ''}
            </span>
          </a>
          ...
          <a href="#/notifications" class="btn-icon btn-ghost" title="Notifications" aria-label="Notifications" style="position: relative;">
            ...
            <span data-unread="notifications" style="position: absolute; top: 6px; right: 6px;">
              ${unreadNotifs > 0 ? `<span style="width: 8px; height: 8px; border-radius: 50%; background: var(--primary); display: block;"></span>` : ''}
            </span>
          </a>
