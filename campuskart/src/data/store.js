 ...
 * Auth connects to backend API; theme/local state persists in localStorage
 */
...
class Store {
  ...
  init() {
    try {
      ...
      console.warn('[Store] localStorage access failed:', err.message);
    }
  }
  ...
  resetToSeed() {
    try {
      ...
      console.warn('[Store] Failed to seed localStorage:', err.message);
    }
  }
  ...
  async login(email, password) {
    ...
    try {
      ...
      console.warn('[Store] localStorage write failed:', err.message);
    }
  ...
  async signup(data) {
    ...
    // Auto-login after signup - need to login to get tokens
  ...
  async updateProfile(updates) {
    const updated = await apiService.updateMyProfile(updates);
    try {
  ...
  // --- Unread counts (fetched from API) ---
  _unreadNotifCount = 0;
  ...
  async refreshUnreadCounts() {
    try {
      const notifResult = await apiService.getUnreadNotificationCount();
      this._unreadNotifCount = notifResult.count;
    } catch {
      this._unreadNotifCount = 0;
    }
    try {
      // Derive unread messages from conversations
      const conversations = await apiService.getConversations();
      this._unreadMessageCount = conversations.reduce((sum, c) => sum + (c.unreadCountForCurrentUser || 0), 0);
    } catch {
      this._unreadMessageCount = 0;
    }
    this.notify('unread_counts_change', {
      notifications: this._unreadNotifCount,
      messages: this._unreadMessageCount,
    });
  }
}
