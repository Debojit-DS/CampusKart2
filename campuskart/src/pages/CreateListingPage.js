...
export async function renderCreateListingPage({ router } = {}) {
  ...
  let uploadedImages = [];
  let categories = []; // fetched from backend
  ...
  try {
    ...
    categories = [
      ...
      { id: 'cat-cycles', slug: 'cycles', label: 'Cycles & Mobility' },
      { id: 'cat-stationery', slug: 'stationery', label: 'Stationery & Supplies' },
    ];
  }
  ...
  fileInput.addEventListener('change', async (e) => {
    ...
    for (const file of files) {
      ...
      previews.insertAdjacentHTML('beforeend', `
        <div id="${loadingId}" style="width: 64px; height: 64px; border-radius: var(--radius-sm); border: 1px dashed var(--primary); display: flex; align-items: center; justify-content: center;">
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const categoryTopId = container.querySelector('#listing-category').value;
    ...
    const imageUrls = listingType === 'item' && uploadedImages.length === 0
