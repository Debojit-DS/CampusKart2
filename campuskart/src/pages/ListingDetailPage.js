...
export async function renderListingDetailPage({ params = {}, router } = {}) {
  ...
  const categoryLabel = typeof listing.categoryTop === 'object' ? listing.categoryTop?.label : listing.categoryTop;
  ...
  container.innerHTML = `
    ${renderTopNavBar({ activeCategory: categorySlug || '', activeRoute: '/listing' })}
    ...
    <main class="container-custom" style="flex: 1; padding-top: 32px; padding-bottom: 64px;">
      ...
      <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--on-surface-variant); margin-bottom: 24px; font-family: var(--font-mono);">
        ...
        <span>&rsaquo;</span>
        <a href="#/category/${categorySlug}" style="color: var(--on-surface-variant);">${escapeHtml(categoryLabel)}</a>
        ${listing.categorySub ? `<span>&rsaquo;</span><span style="color: var(--primary);">${escapeHtml(typeof listing.categorySub === 'object' ? listing.categorySub?.label : listing.categorySub)}</span>` : ''}
      </div>
      ...
      <div>
        <div class="utility-label" style="color: var(--primary); margin-bottom: 6px;">
          ${escapeHtml(categoryLabel || 'Academic')}${listing.categorySub ? ` &bull; ${escapeHtml(typeof listing.categorySub === 'object' ? listing.categorySub?.label : listing.categorySub)}` : ''}
        </div>
                ...
                <div style="display: flex; gap: 10px;">
                  ...
                  >
                   ...
                   <button
                     ...
                     style="width: 100%; color: var(--on-surface-variant); font-size: 12px; margin-top: 4px;"
  ...
  bookmarkBtn?.addEventListener('click', async () => {
    const result = await apiService.toggleBookmark(listing.id);
  ...
  // Report listing
  container.querySelector('#report-listing-btn')?.addEventListener('click', () => {
    openReportModal(listing, async (reason, details) => {
      try {
        await apiService.submitReport({ listingId: listing.id, reason, details });
        toast.success('Report submitted. Thank you for keeping CampusKart safe.');
      } catch (e) {
        toast.error(ErrorHandler.getErrorMessage(e));
      }
    });
...
function openReportModal(listing, onSubmit) {
  ...
  modal.innerHTML = `
    <div class="bg-surface-bright" style="width: 100%; max-width: 440px; padding: 28px; border-radius: var(--radius-lg); border: 1px solid var(--surface-container); position: relative;">
      <button type="button" id="close-report-modal" class="btn-icon btn-ghost" style="position: absolute; top: 12px; right: 12px;">
        <span class="material-symbols-outlined">close</span>
      </button>

      <h2 class="headline-sm" style="margin-bottom: 16px; color: var(--on-surface);">Report Listing</h2>
      <p class="body-md" style="color: var(--on-surface-variant); font-size: 13px; margin-bottom: 20px;">
        Help keep CampusKart safe by reporting inappropriate listings.
      </p>
      ...
      <form id="report-form">
        ...
        <div class="form-group">
          <label class="form-label" for="report-details">Details (optional)</label>
          <textarea id="report-details" class="form-input" rows="3" placeholder="Provide additional details..."></textarea>
        </div>

        <div style="display: flex; gap: 12px; margin-top: 20px;">
          <button type="button" id="cancel-report-btn" class="btn btn-outlined" style="flex: 1;">Cancel</button>
  ...
  modal.querySelector('#close-report-modal').addEventListener('click', close);
  modal.querySelector('#cancel-report-btn').addEventListener('click', close);
